...
protocol PointJobCellCellDelegate:class {
    func clickedFavoritedImageOnPointJobCell(sender: PointJobCell,value: String)
}

class PointJobCell: UICollectionViewCell {
    ...
    weak var delegate: PointJobCellCellDelegate? = nil
    ...
    func updateCellFromDict(dict: NSDictionary) {
        ...
        //Date Title
        if let strTimeString = dict.object(forKey: "updated") as? String {
        ...
        let imageUrl = dict.object(forKey: "image_path") as? String
        self.imageAvatar.setImageForm(urlString: imageUrl, placeHolderImage: nil)

        if let is_favorite = (dict.object(forKey: "job_favorites") as? String) {
            if is_favorite == "1" {
    ...
    @IBAction func clickedButtonFavorited(_ sender: Any) {
        if self.isCallingAPIFavorite == false {
            if let is_favorite = self.infoItem.object(forKey: "job_favorites") as? String {
    ...
    func callAPIFavoriteJobs(currentFavorite: String) {
        if let jobsId: String = self.infoItem.object(forKey: "id") as? String {
            ...
            APIBase.shareObject.callAPIFavoriteJobs(params: params, completionHandler: { (result, error) in
                ...
                if status == 200 {
                    ...
                    self.infoItem.setValue(newValue, forKey: "job_favorites")
                    self.delegate?.clickedFavoritedImageOnPointJobCell(sender: self, value: newValue)
                }
                else {
