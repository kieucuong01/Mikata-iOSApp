...
class PointJobCollectionview: UICollectionViewCell {
    ...
    @IBOutlet weak var listPointJobCollectonView: TPKeyboardAvoidingCollectionView!
    ...
    weak var delegate:PointJobCollectionViewDelegate? = nil
    ...
    override func awakeFromNib() {
        ...
        // Get job category
        self.callAPIGetJobCategory()

        // Set load more and refresh
        self.listPointJobCollectonView.alwaysBounceVertical = true
        self.listPointJobCollectonView.addPullToRefreshHandler {
            self.pageContent = 1
            self.generateParamAndCallAPI()
        }
        self.listPointJobCollectonView.addInfiniteScrollingWithHandler {
    ...
    private func setPropertiesForView() {
        ...
        listPointJobCollectonView.delegate = self
        listPointJobCollectonView.dataSource = self
        listPointJobCollectonView.register(UINib.init(nibName: "PointJobCell", bundle: nil), forCellWithReuseIdentifier: "itemPointJobCell")
        listPointJobCollectonView.register(UINib.init(nibName: "CustomListButtonCell", bundle: nil), forCellWithReuseIdentifier: "customListButtonCell")
    ...
    func generateParamAndCallAPI() {
        if self.pageContent == 1 {
            self.listPointJobCollectonView.setShowsInfiniteScrolling(true)
            self.arrayJobItems.removeAllObjects()
        }

        var params: [String : Any] = [
            "user_id": PublicVariables.userInfo.m_id,
            "name": self.keywordSearching,
        ...
        if self.arrayFilters.indices.contains(self.selectedFilterIndex.section) == true {
            if self.arrayFilters[self.selectedFilterIndex.section].indices.contains(self.selectedFilterIndex.row + 1) == true {
                ...
                case 1:
                    ...
                    break
                case 2:
                    params["job_category"] = filterValue
                    break
                default:
                    break
                }
        ...
        if self.selectedIndex == 1 {
            ...
            params["get_jobs_favorite"] = NSNumber.init(value: 1)
        } else if self.selectedIndex == 3 {
            params["get_jobs_applied"] = NSNumber.init(value: 1)
        }

        self.callAPIGetJobItems(params: params)
    }


    // MARK: - Call API

    func callAPIGetJobItems(params: [String : Any]?) {
        if self.pageContent == 1 {
            NotificationCenter.default.post(name: NSNotificationShowHUDProgress, object: nil)
        }
        APIBase.shareObject.callAPIGetListJob(params: params) { (result, error) in
            ...
            if status == 200 {
                ...
                if (arrays == nil) || (arrays!.count < self.limitContent) {
                    self.listPointJobCollectonView.setShowsInfiniteScrolling(false)
                }
            } else {
            ...
            self.listPointJobCollectonView.reloadSections(indexSet)
            self.listPointJobCollectonView.pullToRefreshView?.stopAnimating()
            self.listPointJobCollectonView.infiniteScrollingView?.stopAnimating()
            NotificationCenter.default.post(name: NSNotificationHideHUDProgress, object: nil)
        }
    }

    func callAPIGetJobCategory() {
        APIBase.shareObject.callAPIGetListJobCaregory(params: nil) { (result, error) in
            let status = (result?.value(forKey: "status") as? NSNumber)?.intValue
            if status == 200 {
...
extension PointJobCollectionview: UICollectionViewDelegate,UICollectionViewDelegateFlowLayout, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 0 {
            ...
            let cell:PointJobCell = collectionView.dequeueReusableCell(withReuseIdentifier: "itemPointJobCell", for: indexPath) as! PointJobCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.section == 1 {
            ...
            self.delegate?.clickedPointJobItemFromSender(sender: self, atIndexPath: indexPath, object: dict)
        }
    }
}

extension PointJobCollectionview:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.selectedFilterIndex.section == 0 &&  self.selectedFilterIndex.row == 0{
            GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_FILTER, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_FILTER_ACTION_CHOOSE_ALL)
        }

        else if self.selectedFilterIndex.section == 1 {
            if self.selectedFilterIndex.row == 0 {
                GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_FILTER, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_FILTER_ACTION_CHOOSE_AREA)
            }
            else  if self.selectedFilterIndex.row == 1{
                GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_FILTER, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_FILTER_ACTION_CHOOSE_CATEGORY)
            }
        }
    }
}

extension PointJobCollectionview: PointJobCellCellDelegate {
    func clickedFavoritedImageOnPointJobCell(sender: PointJobCell, value: String) {
        if self.selectedIndex == 2 {
            ...
            if let dict = self.arrayJobItems[sender.indexPath.row] as? NSDictionary {
                ...
                mulDict.setValue(value, forKey: "job_favorites")
        ...
        // Track Repro
        if value == "1"  {
            GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_ACTION_FAVORITE)
        }
        else {
            GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_ACTION_UNFAVORITE)
        }
    }
}

extension PointJobCollectionview:CustomListButtonCellDelegate {
    func callbackDelegateToClickedButonFirst(sender: CustomListButtonCell?) {
        ...
        GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_ACTION_TAB_ALL)
    ...
    func callbackDelegateToClickedButonSecond(sender: CustomListButtonCell?) {
        ...
        GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_ACTION_TAB_FAVORITE)
    }
    ...
    func callbackDelegateToClickedButonThird(sender: CustomListButtonCell?) {
        ...
        GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_ACTION_TAB_APPLY)
    }

    func callbackDelegateToFilterSelect(sender: CustomListButtonCell?) {
        self.endEditing(true)
        if self.mTableViewListNotifcation.isHidden{
            GlobalMethod.sendEventToRepro(screenName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION, eventName: ReproEvent.REPRO_SCREEN_JOB_INTRODUCTION_ACTION_FILTER)
        }
        self.mTableViewListNotifcation.isHidden = !self.mTableViewListNotifcation.isHidden

        // Update list
        self.listPointJobCollectonView.isScrollEnabled = self.mTableViewListNotifcation.isHidden
        self.listPointJobCollectonView.setContentOffset(CGPoint.zero, animated: true)
    }
...
extension PointJobCollectionview : UIScrollViewDelegate {
